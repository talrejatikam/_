...
<div class="show_if_seller" style="<?php echo esc_attr( $role_style ); ?>">
    ...
    <p class="form-row form-group form-row-wide">
        <label for="shop-phone"><?php esc_html_e( 'Aadhar Card Number', 'dokan-custom-codes' ); ?><span class="required">*</span></label>
        <input type="text" class="input-text form-control" name="uid_id" id="uid_id" value="<?php if ( ! empty( $postdata['uid_id'] ) ) echo esc_attr($postdata['uid_id']); ?>" required="required" />
    </p>
