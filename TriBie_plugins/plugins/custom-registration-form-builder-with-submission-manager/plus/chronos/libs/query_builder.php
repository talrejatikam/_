...
class RM_Chronos_Query_Builder {
    ...
    protected function get_submissions($prime_joiner) {
        ...
        $data = array();
        $query = "SELECT st.submission_id, st.user_email FROM $subs_table st WHERE st.form_id = %d AND st.submission_id IN (SELECT MAX(submission_id) FROM `$subs_table` WHERE `form_id` = %d GROUP BY `user_email`)";
         $data[] =   $this->form_id;
         $data[] =   $this->form_id;
        ...
        $result = $wpdb->get_results($wpdb->prepare($query,$data));
        if(!$result)
            return array();
        else
            return $result;
    ...
    public function get_results($prime_joiner = 'AND') {
        ...
        if(count($this->sub_query_arg) > 0) {
            ...
            $submitters = $wpdb->get_results($wpdb->prepare("SELECT `ID` FROM $user_table WHERE `user_email` IN (%s)",esc_sql($subs_emails)), OBJECT_K);
        } else {
            $subs = $this->get_submissions($prime_joiner);
            $submitters = $wpdb->get_results($wpdb->prepare("SELECT `ID` FROM $user_table WHERE `user_email` IN (SELECT `user_email` FROM $rm_subs_table WHERE `form_id` = %d)",intval($this->form_id)), OBJECT_K);
        }
