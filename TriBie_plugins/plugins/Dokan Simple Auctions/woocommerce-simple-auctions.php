		...
		class WooCommerce_simple_auction {
			...
			public function frontend_enqueue_script() {
				...
				$custom_data                         = array(
					...
					'checking'        => __( 'Patience please, we are checking if auction is finished!', 'wc_simple_auctions' ),
					'gtm_offset'      => get_option( 'gmt_offset' ),
					'started'         => __( 'Auction has started! Please refresh your page.', 'wc_simple_auctions' ),
					'no_need'         => apply_filters('woocommerce_simple_auctions_winning_bid_message', __( 'No need to bid. Your bid is winning! ', 'wc_simple_auctions' ) ),
					...
					?>
							...
							foreach ( $auction_history as $history_value ) {
								if ( $history_value->date < $product_data->get_auction_relisted() &&
									...
									echo '</table>';
									?>
									<h2 class="old_auctions_data"><?php _e( 'Auction Data Prior Relist', 'wc_simple_auctions' ); ?> </h2>
									<table class="auction-table widefat fixed">
									<thead>
										<tr>
											<th><?php _e( 'Date', 'wc_simple_auctions' ); ?></th>
...
?>
			...
			public function pre_get_posts( $q ) {
				...
				switch ( $orderby_value ) {
					...
					case 'bid_asc':
						...
						$meta_query[] = array( 'bid_asc' => array(
								'relation'            => 'OR',
								'auction_start_price' => array(
									'key'  => '_auction_start_price',
									'type' => 'DECIMAL(32,4)',
						...
						foreach ( $order_items as $item_id => $item ) {
							...
							if ( method_exists( $product_data, 'get_type' ) && $product_data->get_type() == 'auction' ) {
								...
								if ( ! $product_data->is_finished() ) {
									...
									update_post_meta( $product_id, '_auction_dates_to', date( 'Y-m-d h:s' ) );
					...
					if ( $product_data->is_closed() ) {
						...
						if ( isset( $_POST['ret'] ) && $_POST['ret'] != '0' ) {
							...
							if ( $product_data->is_reserved() ) {
								...
								$return['message'] .= sprintf( __( 'Winning bid is %1$s by %2$s.', 'wc_simple_auctions' ), wc_price( $product_data->get_curent_bid() ), apply_filters( 'woocommerce_simple_auctions_displayname', get_userdata( $product_data->get_auction_current_bider() )->display_name, $product_data ) );
			...
			*/
			...
			$header_row = array(
				'auction_id'    => __( 'Auction id', 'wc_simple_auctions' ),
				'auction_title' => __( 'Auction title', 'wc_simple_auctions' ),
				'userid'        => __( 'Bider', 'wc_simple_auctions' ),
				'user_email'    => __( 'Bider email', 'wc_simple_auctions' ),
				'bid'           => __( 'Bid', 'wc_simple_auctions' ),
				'date'          => __( 'Date', 'wc_simple_auctions' ),
				'proxy'         => __( 'Proxy', 'wc_simple_auctions' )
	        );
